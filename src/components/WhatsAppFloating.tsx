...
export default function WhatsAppFloating() {
  const message = `Hello ${brandInfo.shortName}, I am interested in your commercial carpet solutions. Could you provide more information?`;
  const whatsappUrl = `https://wa.me/${brandInfo.whatsapp.replace(/\s+/g, "")}?text=${encodeURIComponent(message)}`;

  return (
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col items-center gap-2 group">
      <div className="bg-[#102A43] text-white text-[10px] font-black py-2 px-4 rounded-sm uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all shadow-xl -translate-x-4 group-hover:translate-x-0 whitespace-nowrap">
        WhatsApp Business
      </div>
      <a
        href={whatsappUrl}
        ...
        className="w-16 h-16 bg-[#25D366] text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-all duration-300 relative"
      >
