            ...
            <div className="grid gap-5 sm:grid-cols-3">
              {relatedProducts.map((item) => (
                <Link key={item.id} href={productPath(item.id)} className="group border border-border bg-white p-4 transition-all hover:border-accent hover:shadow-lg">
                  ...
                  <p className="text-sm font-bold leading-snug text-primary group-hover:text-accent">{item.name}</p>
                </Link>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-4">
              {guides.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="block border border-border bg-surface p-5 transition-all hover:border-accent hover:bg-white">
                  ...
                  <p className="text-sm font-bold leading-snug text-primary">{post.title}</p>
                </Link>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-border bg-surface p-5">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.08em] text-accent">Quick Factory Quote</p>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <a
          ...
          className="border border-border bg-surface px-4 py-4 text-center text-[13px] font-bold uppercase tracking-[0.06em] text-primary transition-all hover:border-accent hover:bg-white"
        >
    ...
    <div className="grid gap-3 sm:grid-cols-2" data-funnel-section="procurement_facts">
      {specCards(product).map((item) => (
        <div key={item.label} className="border border-border bg-white p-4 shadow-sm">
          <p className="mb-2 text-xs font-bold uppercase tracking-[0.08em] text-muted">{item.label}</p>
          <p className="font-mono text-sm font-bold leading-relaxed text-primary">{item.value}</p>
        </div>
      ))}
    </div>
  );
      ...
      <div className="container-fox">
        ...
        <div className="grid gap-px border border-border bg-border md:grid-cols-5">
          {reasons.map((reason) => (
            <div key={reason.title} className="bg-white p-6">
              <h3 className="mb-3 text-sm font-bold uppercase tracking-[0.08em] text-primary">{reason.title}</h3>
        ...
        <div className="mt-10 border border-primary bg-white p-6 md:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              <p className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-accent">Request Quote Package</p>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              <Link
                ...
                className="flex min-h-12 items-center justify-center bg-[#d9480f] px-5 py-4 text-center text-[13px] font-bold uppercase tracking-[0.06em] text-white transition-all hover:bg-[#b83a08]"
              >
                Send Inquiry
              </Link>
              <Link
