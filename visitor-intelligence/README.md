...
wrangler d1 create VCARPETS_visitors
...
```bash
wrangler d1 execute VCARPETS_visitors --remote --file=./schema.sql
```

For an existing installation, apply the visitor-correlation migration once:

```bash
wrangler d1 execute VCARPETS_visitors --remote \
...
## 2. Connect VCARPETSCarpet

Add the Worker URL to Vercel:

```bash
NEXT_PUBLIC_VISITOR_BEACON_URL=https://VCARPETS-visitor-intelligence.<your-subdomain>.workers.dev
VISITOR_INGEST_URL=https://VCARPETS-visitor-intelligence.<your-subdomain>.workers.dev/visit
```
...
```bash
LEAD_INGEST_URL=https://VCARPETS-visitor-intelligence.<your-subdomain>.workers.dev/lead
LEAD_INGEST_SECRET=<same random value stored in the Worker secret>
```

The browser posts to VCARPETS's same-origin `/api/lead` route. That route sends the inquiry to Formspree and archives it in D1 using the private Worker secret; the secret is never exposed to the browser.
...
```bash
wrangler d1 execute VCARPETS_visitors --remote --command "SELECT * FROM visits ORDER BY id DESC LIMIT 10"
```
...
```bash
wrangler d1 execute VCARPETS_visitors --remote --json \
...
The files are written to `~/Downloads/VCARPETS-Lead-Reports/<date>/`:
...
```bash
wrangler d1 execute VCARPETS_visitors --remote --command \
